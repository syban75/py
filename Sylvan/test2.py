...
def check_naver_alarm(driver):
    ...
    try:
        ...
        if alarm_time != config.last_alarm_time:
            """ 클릭보다 바로 접근
            # 지금 신청하기 클릭
            ...
            item.find_element_by_class_name("btn_basic").get_attribute("href")
...
def naver_login2(nid, npw):
    encnm, encpw = encrypt(nid, npw)
    bvsd_uuid = uuid.uuid4()
    o = '{"a":"' + str(
        bvsd_uuid) + '","b":"1.3.4","h":"1f","i":{"a":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.163 Whale/2.7.100.20 Safari/537.36"}}'
...
def check_naver_alarm3(driver, session):
    logger = logging.getLogger("check_naver_alarm")
    try:

        # 네이버 알림화면에서 새로운 알림 확인
        resp_html = session.get('https://talk.naver.com/ct/w4ocfa')

        soup = BeautifulSoup(resp_html.text, 'lxml')

        ele = soup.find("script", {"id": "__NEXT_DATA__"})

        print(ele.decode_contents())
...
my_driver = login_naver_alarm()

my_session = naver_login2(config.CONFIG['naver_id'], config.CONFIG['naver_pw'])

check_naver_alarm3(my_driver, my_session)
